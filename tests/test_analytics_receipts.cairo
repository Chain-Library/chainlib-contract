...
use chain_lib::chainlib::ChainLib::ChainLib::{
    ContentType, Category, SalesMetrics, PurchaseAnalytics, ConversionMetrics, 
    Receipt, ReceiptStatus, TimeBasedMetrics, CreatorMetrics
};
...
fn create_test_creator(
    ...
    let creator_id = dispatcher.register_user(username, Role::WRITER, Rank::BEGINNER, 'test_creator');
    creator_id
}
...
fn create_test_content(
    ...
    cheat_caller_address(contract_address, creator_address, CheatSpan::Indefinite);
    let content_id = dispatcher.register_content(
        title,
        'test_description',
        ContentType::Text,
        Category::Education
...
fn test_content_sales_metrics() {
    ...
    // Create purchases
    create_completed_purchase(dispatcher, contract_address, admin_address, buyer1_address, content_id);
    create_completed_purchase(dispatcher, contract_address, admin_address, buyer2_address, content_id);
...
fn test_creator_metrics() {
    ...
    // Create purchase
    create_completed_purchase(dispatcher, contract_address, admin_address, buyer_address, content_id);
...
fn test_receipt_signature_verification() {
    ...
    let is_signature_valid = dispatcher.verify_receipt_signature(
        receipt.receipt_id, receipt.signature
    );
    assert(is_signature_valid, 'Signature should be valid');
    
    // Test with wrong signature
    let wrong_signature_valid = dispatcher.verify_receipt_signature(
        receipt.receipt_id, 'wrong_signature'
    );
