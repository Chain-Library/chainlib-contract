...
mod permission_tests {
    ...
    fn test_set_and_get_operator_permissions() {
        ...
        assert(operator_permissions.value == (permission_flags::READ | permission_flags::EXECUTE), 'wrong perm');
        
        // Verify the operator has specific permissions
        let has_read = dispatcher.has_permission(account_id, operator_address, permission_flags::READ);
        let has_execute = dispatcher.has_permission(account_id, operator_address, permission_flags::EXECUTE);
        let has_write = dispatcher.has_permission(account_id, operator_address, permission_flags::WRITE);
        ...
        let has_read_after = dispatcher.has_permission(account_id, operator_address, permission_flags::READ);
    ...
    fn test_modify_account_permissions() {
        ...
        let has_write = (updated_account.owner_permissions.value & permission_flags::WRITE) == permission_flags::WRITE;
    ...
    fn test_multiple_operators() {
        ...
        dispatcher.set_operator_permissions(
            account_id, 
            operator1, 
            Permissions { value: permission_flags::READ }
        );
        
        dispatcher.set_operator_permissions(
            account_id, 
            operator2, 
            Permissions { value: permission_flags::EXECUTE }
        );
        
        dispatcher.set_operator_permissions(
            account_id, 
            operator3, 
            Permissions { value: permission_flags::WRITE }
        );
    ...
    fn test_permission_combinations() {
        ...
        let test_combinations = array![
            ...
            (permission_flags::MANAGE_PERMISSIONS | permission_flags::MANAGE_OPERATORS, 'MANAGE combo')
        ];
    ...
    fn test_nonexistent_account() {
        ...
        let has_read = dispatcher.has_permission(nonexistent_account_id, operator, permission_flags::READ);
