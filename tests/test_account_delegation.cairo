...
fn setup() -> ContractAddress {
    ...
    let calldata = array![next_namespace.into(), account.into()];
...
fn test_constructor() {
    ...
    let non_owner = contract_address_const::<0xadb>();
    ...
    assert(contract_instance.is_owner(owner), 'Owner should be set');
...
fn test_delegate_permission_past_expiry() {
    ...
    // Try to delegate with expiry in the past
    let past_expiry: u64 = current_time - 100;
    contract_instance.delegate_permission(delegate, PERMISSION_TRANSFER, past_expiry, 0);
}
