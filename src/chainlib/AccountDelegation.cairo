...
pub mod AccountDelegation {
    ...
    impl AccountDelegationImpl of IAccountDelegation<ContractState> {
        ...
        fn delegate_permission(
            ...
            assert(!delegate.is_zero(), 'InvalidDelegation');

            // assert(self.is_valid_permission(permission_id), 'InvalidPermission');
            assert(self.is_valid_permission(permission_id), 'InvalidPermission');
            ...
            if expiry != 0 {
                // assert(expiry > current_time, 'InvalidExpiry');
                assert(expiry > current_time, 'InvalidExpiry');
            }
        ...
        fn revoke_delegation(
            ...
            assert(delegation_info.delegate == delegate, 'NotAuthorized');
        ...
        fn use_delegation(ref self: ContractState, owner: ContractAddress, permission_id: u8) {
            ...
            let caller = get_caller_address();
            let namespace = self.owner_namespaces.read(owner);
            assert(namespace != 0, 'NotAuthorized');

            // Get delegation info
            let mut delegation = self.delegations.read((namespace, permission_id));

            // Verify delegation
            assert(delegation.delegate == caller, 'NotAuthorized');
            assert(delegation.active, 'DelegationNotActive');
            ...
            if delegation.expiry != 0 && delegation.expiry <= current_time {
                ...
                assert(delegation.expiry > current_time, 'DelegationExpiry');
            }

            // Check action limit
            if delegation.max_actions != 0 {
                assert(delegation.action_count < delegation.max_actions, 'ActionLimitReached',);
    ...
    impl HelperImpl of HelperTrait {
        fn assert_is_owner(self: @ContractState, address: ContractAddress) {
            assert(self.owners.read(address), 'NotAuthorized');
        }
